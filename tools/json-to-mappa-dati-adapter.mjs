...
function validateInput(data, filePath) {
  ...
  if (data.unitaApprendimento.length === 0) {
    error(
      filePath + ': unitaApprendimento è vuoto — nessuna unità trasformabile'
    )
  }
  for (let i = 0; i < data.unitaApprendimento.length; i++) {
    const u = data.unitaApprendimento[i]
    if (!u.id) warn('Unità #' + i + ' senza id')
    if (!u.ambito || u.ambito === '')
      warn('Unità ' + (u.id || '#' + i) + ': ambito mancante o vuoto')
    if (!u.nucleo || u.nucleo === '')
      warn('Unità ' + (u.id || '#' + i) + ': nucleo mancante o vuoto')
    if (u.ordine === 'Infanzia' && (!u.fascia || u.fascia === ''))
      warn('Unità ' + (u.id || '#' + i) + ' Infanzia: fascia mancante')
    if (
      (u.ordine === 'Primaria' || u.ordine === 'Secondaria') &&
      (u.classe === null || u.classe === '')
    )
      warn('Unità ' + (u.id || '#' + i) + ' ' + u.ordine + ': classe mancante')
  }
}
...
function buildNodiDisciplinari(units) {
  ...
  for (const u of units) {
    let key =
      u.nucleoFondante && u.nucleoFondante !== '' ? u.nucleoFondante : u.nucleo
    if (!key || key === '') {
      warn(
        'Unità ' +
          u.id +
          ': impossibile determinare nucleo — skip per nodiDisciplinari'
      )
      continue
    }
    if (!groups[key]) {
