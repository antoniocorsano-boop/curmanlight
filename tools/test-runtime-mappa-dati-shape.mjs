...
function normalizeJsonText(raw, label) {
  if (raw.includes('\u0000')) {
    throw new Error(
      `${label}: byte nulli rilevati nell'output JSON (possibile UTF-16 o encoding non valido)`
    )
  }
...
function testPipeline(filepath) {
  ...
  for (let i = 0; i < s.length; i++) {
    ...
    check(
      s[i].descrizione,
      `struttureSostanziali[${i}].descrizione mancante`,
      errors
    )
  ...
  for (let i = 0; i < n.length; i++) {
    ...
    check(
      n[i].descrizione,
      `nodiDisciplinari[${i}].descrizione mancante`,
      errors
    )
  ...
  for (let i = 0; i < p.length; i++) {
    ...
    check(
      p[i].fascia || p[i].classe,
      `progressioneVerticale[${i}]: almeno uno tra fascia e classe richiesto`,
      errors
    )
    check(
      p[i].descrizioneProgressione,
      `progressioneVerticale[${i}].descrizioneProgressione mancante`,
      errors
    )
  ...
  check(
    Array.isArray(data.decisioniCurricolari),
    'decisioniCurricolari non è array',
    errors
  )
  for (let i = 0; i < d.length; i++) {
    ...
    check(
      d[i].motivazione,
      `decisioniCurricolari[${i}].motivazione mancante`,
      errors
    )
  ...
  if (expected) {
    check(
      s.length === expected.s,
      `struttureSostanziali attese ${expected.s}, trovate ${s.length}`,
      errors
    )
    check(
      n.length === expected.n,
      `nodiDisciplinari attesi ${expected.n}, trovati ${n.length}`,
      errors
    )
    check(
      p.length === expected.p,
      `progressioneVerticale attesa ${expected.p}, trovata ${p.length}`,
      errors
    )
    check(
      d.length === expected.d,
      `decisioniCurricolari attese ${expected.d}, trovate ${d.length}`,
      errors
    )
  }
  return {
    discipline,
    errors,
    counts: `S=${s.length} N=${n.length} P=${p.length} D=${d.length}`,
  }
}
