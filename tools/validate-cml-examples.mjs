...
const failures = [];
let checkedFiles = 0;
let managedTeacherFiles = 0;
let legacyTeacherFiles = 0;
...
function validateManagedProposal(file, proposal, index, ids) {
...
async function main() {
  ...
  if (files.length === 0) throw new Error(`Nessun file .cml trovato in ${examplesDir}`);

  for (const file of files) {
    checkedFiles += 1;
    let doc;
    try {
      doc = JSON.parse(await readFile(path.join(examplesDir, file), 'utf8'));
    } catch (error) {
    ...
    const managed = file.startsWith('proposta_');
    if (!managed) {
      legacyTeacherFiles += 1;
      if (doc.schemaVersion !== '1.0') fail(file, 'schemaVersion legacy deve essere "1.0"');
      if (doc.humanValidationRequired !== true) fail(file, 'humanValidationRequired deve essere true');
      if (!Array.isArray(doc.proposals)) fail(file, 'proposals deve essere un array');
      continue;
    }

    managedTeacherFiles += 1;
    ...
    const ids = new Set();
    doc.proposals.forEach((proposal, index) => validateManagedProposal(file, proposal, index, ids));
  }

  if (managedTeacherFiles === 0) fail('libreria', 'nessun teacher_proposal gestito proposta_*.cml trovato');
  ...
  console.log(
    `CML example validation PASS: ${checkedFiles} file, ${managedTeacherFiles} proposte gestite, ${legacyTeacherFiles} teacher proposal legacy, ${checkedProposals} record verificati`,
  );
}
