...
for (const file of FILES) {
  ...
  try {
    ...
    results.push({
      file,
      valid: false,
      error: `Impossibile leggere ${file}: ${e.message}`,
    })
    continue
  }
  ...
  if (data.schemaVersion !== 'cml-normalized-curriculum-v1') {
    errors.push(
      `schemaVersion atteso "cml-normalized-curriculum-v1", trovato "${data.schemaVersion}"`
    )
  }
  ...
  if (hasNucleo && !hasNucleoFondante) {
    warnings.push(
      `Campo "nucleo" presente ma "nucleoFondante" assente (retrocompatibile)`
    )
  }
  if (!hasNucleo && hasNucleoFondante) {
    warnings.push(
      `Campo "nucleoFondante" presente ma "nucleo" assente (retrocompatibile)`
    )
  }
  if (hasNucleo && hasNucleoFondante) {
    warnings.push(
      `Entrambi i campi "nucleo" e "nucleoFondante" presenti — uniformare in futuro`
    )
  }
  ...
  if (!Array.isArray(data.unitaApprendimento)) {
    ...
    for (const unit of data.unitaApprendimento) {
      const unitMissing = REQUIRED_FIELDS.filter(
        (f) => !Object.prototype.hasOwnProperty.call(unit, f)
      )
      if (unitMissing.length > 0)
        missing.push({ id: unit.id || 'SENZA_ID', missing: unitMissing })
      ...
      const hasNucleoFondante = Object.prototype.hasOwnProperty.call(
        unit,
        'nucleoFondante'
      )
      if (hasNucleo && !hasNucleoFondante) {
        warnings.push({
          id: unit.id || 'SENZA_ID',
          msg: `Campo "nucleo" presente ma "nucleoFondante" assente (retrocompatibile)`,
        })
      }
      if (!hasNucleo && hasNucleoFondante) {
        warnings.push({
          id: unit.id || 'SENZA_ID',
          msg: `Campo "nucleoFondante" presente ma "nucleo" assente (retrocompatibile)`,
        })
      }
      if (hasNucleo && hasNucleoFondante) {
        warnings.push({
          id: unit.id || 'SENZA_ID',
          msg: `Entrambi i campi "nucleo" e "nucleoFondante" presenti — uniformare in futuro`,
        })
      }
      ...
      if (!VALID_ORDERS.has(unit.ordine))
        warnings.push({
          id: unit.id || 'SENZA_ID',
          msg: `Ordine non standard: ${unit.ordine}`,
        })
      if (unit.stato && !VALID_STATES.has(unit.stato))
        warnings.push({
          id: unit.id || 'SENZA_ID',
          msg: `Stato non standard: ${unit.stato}`,
        })
    ...
    for (const o of requiredOrders) {
      if (!orders.has(o))
        warnings.push({ id: 'FILE', msg: `Ordine mancante: ${o}` })
    }

    const requiredPri = ['1', '2', '3', '4', '5']
    if (primariaClasses.size > 0) {
      for (const c of requiredPri) {
        if (!primariaClasses.has(c))
          warnings.push({ id: 'FILE', msg: `Primaria classe ${c} mancante` })
      }
    }
    ...
    if (secondariaClasses.size > 0) {
      for (const c of requiredSec) {
        if (!secondariaClasses.has(c))
          warnings.push({ id: 'FILE', msg: `Secondaria classe ${c} mancante` })
      }
    }

    if (primariaClasses.size === 0)
      warnings.push({ id: 'FILE', msg: 'Nessuna unità primaria' })
    if (secondariaClasses.size === 0)
      warnings.push({ id: 'FILE', msg: 'Nessuna unità secondaria' })
  }

  totalUnits += Array.isArray(data.unitaApprendimento)
    ? data.unitaApprendimento.length
    : 0
