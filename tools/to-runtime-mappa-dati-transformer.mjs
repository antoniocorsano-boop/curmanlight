...
function toRuntimeMappaDati(data) {
  ...
  if (data.struttureSostanziali)
    output.struttureSostanziali = transformStruttureSostanziali(
      data.struttureSostanziali
    )
  if (data.nodiDisciplinari)
    output.nodiDisciplinari = transformNodiDisciplinari(data.nodiDisciplinari)
  if (data.progressioneVerticale)
    output.progressioneVerticale = transformProgressioneVerticale(
      data.progressioneVerticale
    )
  if (data.decisioniCurricolari)
    output.decisioniCurricolari = transformDecisioniCurricolari(
      data.decisioniCurricolari
    )
  return output
}
