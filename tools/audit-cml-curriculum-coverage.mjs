...
const runtimePath =
  process.argv[2] || '_published_snapshot/netlify-current/index.html'
...
const disciplineMetaBlock = between(
  html,
  'const DISCIPLINE_META = {',
  '};\nconst DISCIPLINE'
)
const dataBlock = between(
  html,
  'let DATA = {',
  '\n};\n\n// ================================================================\n// ================================================================\nlet selDisc'
)

const rawMetaDisciplines = [
  ...disciplineMetaBlock.matchAll(/^\s*"([^"]+)"\s*:/gm),
].map((m) => m[1])
const metaDisciplineCounts = rawMetaDisciplines.reduce((acc, name) => {
  acc[name] = (acc[name] || 0) + 1
  return acc
}, {})
const duplicatedMetaDisciplines = Object.entries(metaDisciplineCounts)
...
const dataDisciplines = [...dataBlock.matchAll(/^"([^"]+)"\s*:\s*\{/gm)].map(
  (m) => m[1]
)
...
const coverage = dataDisciplines.map((name) => {
  ...
  const byOrder = Object.fromEntries(
    expectedOrders.map((order) => [
      order,
      countIn(fragment, new RegExp(`ordine:\\"${order}\\"`, 'g')),
    ])
  )
  const stati = {
  ...
  return {
    disciplina: name,
    traguardi: countIn(fragment, /traguardi:\[/g)
      ? countIn(fragment, /id:\"[^\"]+\"/g) -
        countIn(
          fragment.match(/obiettivi:\[[\s\S]*/)?.[0] || '',
          /id:\"[^\"]+\"/g
        )
      : 0,
    obiettivi: countIn(fragment, /obiettivi:\[[\s\S]*/)?.[0]
      ? countIn(
          fragment.match(/obiettivi:\[[\s\S]*/)?.[0] || '',
          /id:\"[^\"]+\"/g
        )
    ...
    ordiniConVoci: Object.entries(byOrder)
      .filter(([, v]) => v > 0)
      .map(([k]) => k),
    ordiniVuoti: Object.entries(byOrder)
      .filter(([, v]) => v === 0)
      .map(([k]) => k),
    stati,
...
const consistencyWarnings = coverage
  .filter((d) => d.statusTotal !== d.totaleVoci)
  .map((d) => ({
...
const result = {
  ...
  metadataOnlyDisciplines: metaDisciplines.filter(
    (d) => !dataDisciplines.includes(d)
  ),
  dataWithoutMetadata: dataDisciplines.filter(
    (d) => !metaDisciplines.includes(d)
  ),
  coverage,
  consistencyWarnings,
  schemaFinding:
    'Il runtime corrente modella traguardi e obiettivi. Non emerge una struttura dati sistematica per ambito, competenza, conoscenze e abilità su ogni disciplina.',
  uxFinding:
    'La UI espone molte azioni e pulsanti nello stesso spazio operativo; serve un audit orientato a una schermata/una decisione e a un livello progressivo di dettaglio.',
  uiDensity,
}
