...
function parseArgs() {
  ...
  if (!jsonPath) {
    process.stderr.write(
      'Uso: node tools/generate-static-mappa-dati.mjs <json-path> [--const NOME]\n'
    )
    process.exit(1)
  }
...
function inferConstName(disciplina) {
  ...
  return (
    map[disciplina] ||
    disciplina.toUpperCase().replace(/\s+/g, '_') + '_MAPPA_DATI'
  )
}
...
function run() {
  ...
  const adapter = spawnSync('node', [ADAPTER, absPath], {
  ...
  try {
    ...
    process.stderr.write(
      `[ERROR] Output transformer non è JSON valido: ${e.message}\n`
    )
    process.exit(1)
  }
