...
function isRoleAccessGranted() {
  try {
    return sessionStorage.getItem(ROLE_ACCESS_STORAGE_KEY) === 'true'
  } catch (_err) {
    return false
  }
}
...
function lockRoleAccess() {
  try {
    sessionStorage.removeItem(ROLE_ACCESS_STORAGE_KEY)
  } catch (_err) {}
...
<p>
  Questa funzione è riservata al ruolo indicato nella prova controllata.
  Inserisci il codice operativo fornito dal referente.
</p>
<p class="role-access-warning">
  Il codice evita usi accidentali dello strumento. Non è una password
  istituzionale e non sostituisce autorizzazioni, firme o delibere.
</p>
```
